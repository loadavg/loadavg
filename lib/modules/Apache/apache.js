...
charts.apache_usage = 
{
	...
	options: 
	{
		...
		tooltipOpts: {
			...
			content: function(label, xval, yval, flotItem) {
				...
				return "CPU Usage : " + parseFloat(yval).toFixed(4);
	    	},
	...
	setRange: function($data1,$data2,$data3)
	{
		...
		this.chartBarWidth       = 60*3*1000;
  		...
  		console.log(  $data3, "setRange in apache!");
	...
	init: function($divId)
	{
		...
		if (this.$data[0]) { 
			if ( myTicks  == 12 ) {
				loadchart[0] = [ {	label: this.$data[0].label, data: this.$data[0].data,
		...
		if (this.$data[1]) { 
			loadchart[1] = [ {	label: this.$data[1].label, data: this.$data[1].data,
				     			lines: {show: false},
		...
		if ($.isArray(this.$data))
		{
			if (this.$data[0] && this.$data[1])  {
				this.plot = $.plot('#' + $divId, loadchart[0].concat(loadchart[1] ), this.options);
				//console.log( 'apache level 2' );

			} else if (this.$data[0])  {
				this.plot = $.plot('#' + $divId, loadchart[0], this.options);
