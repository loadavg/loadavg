...
$(function(){
	...
	$('#myModal').on('show', function(){ //subscribe to show method
		...
		//get the data
		thedata = getTimeSlotAlert(module,time);
...
function getTimeSlotAlert(module,time) {
	...
	for (var i = 0; i < alertData[module].length; ++i) {
		...
		//console.log('Get time: ' ,getHumanDate(getTime));
		...
		if ( (getTime >= startTime) && (getTime < endTime)  ) {
			...
			//console.log('Alert : ' ,theAlert);

			thedata[loop] =  '<span style="color:#4F2817;"><strong>' + getHumanDate(getTime) + '</strong>  '  
							//+ alertData[module][i][1] + ' '  
							+ '<strong>Alert</strong> ' + theAlert[0][0] + ' '
							+ '<strong>Trigger set at</strong> ' + theAlert[0][1] + ' '
							+ '<strong>Value recorded</strong> ' + theAlert[0][2] + '<span><br>';
