...
function getTimeSlotAlert(module,time,span) {
    ...
    //get the date
    var theDate = getHumanDate(time);
    //console.log ('theDate : ', theDate)

    //set start time
	startTime = time;
	
    //console.log('Start time: ' ,getHumanTime(startTime));
    ...
    //console.log('End time: ' ,getHumanTime(endTime));

	var thedata = [];
	thedata[0] = '<strong>' + module + ' alerts</strong> <strong>from</strong> ' 
			   + getHumanTime(startTime) + ' <strong>to</strong> ' 
			   + getHumanTime(endTime) + ' on ' + theDate + '<br>';
	...
	for (var i = 0; i < alertData[module].length; ++i) {
		...
		if ( (getTime >= startTime) && (getTime < endTime)  ) {
			...
			theAlertTime = getHumanTime(getTime);
...
var dynamicTable = (function() {
    ...
    function _buildRowColumns(names, item, itemnumber) {
        ...
        if (names && names.length > 0)
        {
			...
			var timeStamp = 0;
            if (typeof dataArray[itemnumber+1].timeStamp != 'undefined') {
				timeStamp = dataArray[itemnumber+1].timeStamp;
            }
...
$(document).ready(function(e) {
    ...
    //need to build dataArray here instead of in php
    ...
    /*
     * now create core of chart using dataArray (sent from php side)
     */

    var chartDataArray = [];
    chartDataArray = buildChartCore(dataArray, modules);


    /*
     * render chart object with data from dataArray
	 */
